...
type Row = { id:number; user_id:number; requested_at:string; purge_after:string; status:string }
...
export default function AdminDeletionsPage() {
  ...
  useEffect(() => {
    try { setKey(localStorage.getItem('tregu:admin_key') || '') } catch {}
  }, [])
  ...
  async function restore(user_id:number) {
    setError(null)
    try {
      const r = await fetch(`${API}/admin/deletions/restore?user_id=${user_id}`, { method:'POST', headers:{'X-Admin-Key':key} })
  ...
  async function forceExport(user_id:number) {
    setError(null)
    try {
      const r = await fetch(`${API}/admin/deletions/export?user_id=${user_id}`, { method:'POST', headers:{'X-Admin-Key':key} })
    ...
    <div className="space-y-6">
      ...
      <div className="rounded-xl border border-slate-200">
        <div className="grid grid-cols-5 gap-2 p-3 border-b text-xs font-medium text-slate-600">
          <div>ID</div><div>User</div><div>Requested</div><div>Purge after</div><div>Actions</div>
        </div>
        {rows.length===0 ? (
