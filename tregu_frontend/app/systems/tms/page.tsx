...
export default function TMSPage() {
  ...
  return (
    <div className="space-y-4">
      <h1 className="text-2xl font-semibold">Shipping (TMS)</h1>
      {isTest && <div className="rounded-lg bg-amber-50 p-3 text-amber-700">Test Mode</div>}
      <p className="text-slate-600">Rate shop, labels, manifest, tracking.</p>
      <div className="flex gap-2">
        <Link className="btn" href="/systems">Back to Systems</Link>
        <Link className="btn btn-primary" href="/systems/tms?mode=test">Buy a test label</Link>
      </div>
    </div>
  );
}