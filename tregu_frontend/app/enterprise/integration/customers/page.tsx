...
export default function MapCustomers() {
  ...
  async function upload() {
    ...
    const r = await fetch("/api/proxy/integration/upload/customers", {
      method: "POST",
      body: fd,
    });
  ...
  async function validate() {
    ...
    const r = await fetch("/api/proxy/integration/map-validate/customers", {
      method: "POST",
      body: fd,
    });
    const j = await r.json();
  ...
  async function apply() {
    const fd = new FormData();
    fd.append("batch_id", batchId);
    const r = await fetch("/api/proxy/integration/apply/customers", {
      method: "POST",
      body: fd,
    });
    const j = await r.json();
  ...
  return (
    <div className="p-6 space-y-4">
      <h1 className="text-2xl font-semibold">Customer Import Mapping</h1>
